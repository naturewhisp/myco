...
object SpeciesCatalog {
    val all: List<MushroomSpecies> = listOf(
        ...
        species("armillaria_mellea", "Armillaria mellea (Vahl) P. Kumm.", "Chiodino", EcologicalCategory.PARASITIC, 50, 1300, 150, 1000, 9.0, 17.0, 5.0, 20.0, 30.0, listOf("latifoglie", "ceppaie", "boschi misti"), "Autunno inoltrato a cespi alla base dei tronchi", listOf(8, 9, 10), listOf("Hypholoma fasciculare (falso chiodino, tossico amaro)", "Galerina marginata (mortale)"), "Tossico da crudo: necessita di pre-bollitura prolungata (almeno 20 min) gettando l'acqua di cottura."),
        species("lactarius_deliciosus", "Lactarius deliciosus (L.) Gray", "Sanguinello / Fungo del pino", EcologicalCategory.ECTOMYCORRHIZAL, 200, 1700, 400, 1300, 10.0, 18.0, 6.0, 22.0, 25.0, listOf("pinus"), "Inizio autunno in pinete montane e collinari", listOf(8, 9, 10, 11), listOf("Lactarius torminosus (Peveraccio delle coliche, tossico grave)"), "Verificare lattice color carota/arancio immutabile o che vira al rosso/verde; scartare specie a lattice bianco acre."),
        species("morchella_esculenta", "Morchella esculenta (L.) Pers.", "Spugnola comune", EcologicalCategory.SAPROTROPHIC, 100, 1500, 200, 1000, 10.0, 18.0, 5.0, 22.0, 30.0, listOf("fraxinus", "ulmus", "populus", "radure", "frassino", "olmo"), "Fruttificazione primaverile (Marzo-Maggio) su suoli alcalini e radure", listOf(2, 3, 4), listOf("Gyromitra esculenta (Falsa spugnola, mortale da cruda, contiene giromitrina)"), "Tossica da cruda (contiene emolisine termolabili): necessita di cottura prolungata (almeno 20-25 min) o preventiva essiccazione."),
    )
    ...
    ) = MushroomSpecies(id, binomialName, vernacularName, category, minElevation, maxElevation, idealElevationMin, idealElevationMax, idealTempMin, idealTempMax, toleratedTempMin, toleratedTempMax, minRainAccumulation, preferredCanopyTypes, fruitingPeriodDescription, activeMonths, toxicLookAlikes, edibilityWarning)
}
